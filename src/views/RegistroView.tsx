...
interface RegistroViewProps {
  page: RegistroPage | null;
}

export default function RegistroView({ page }: RegistroViewProps) {
  const title = page?.title ?? "Registro fidelización";
  const description = page?.description ?? "Completa el formulario para unirte al programa.";
  const formSrc = page?.zohoFormUrl ?? FALLBACK_FORM_SRC;
